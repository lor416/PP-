...
public class Book {
    ...
    public Book() {
        this.markList = new ArrayList<WriteMark>();
    }
    public static class WriteMark{
    ...
    public static void saveAllToJson(List<Book> students, String filename) {
        try {
            ObjectMapper mapper = new ObjectMapper();
            mapper.enable(SerializationFeature.INDENT_OUTPUT); // красивый формат

            File file = new File(filename);
            mapper.writeValue(file, students);

            System.out.println("Все студенты сохранены в файл: " + filename);
        } catch (Exception e) {
            System.err.println("Ошибка при сохранении в JSON: " + e.getMessage());
        }
    }

    // Простой метод для чтения ВСЕХ студентов из JSON файла
    public static List<Book> loadAllFromJson(String filename) {
        try {
            ObjectMapper mapper = new ObjectMapper();

            File file = new File(filename);
            List<Book> students = mapper.readValue(file,
                    mapper.getTypeFactory().constructCollectionType(List.class, Book.class));

            System.out.println("Студенты загружены из файла: " + filename);
            return students;
        } catch (Exception e) {
            System.err.println("Ошибка при чтении из JSON: " + e.getMessage());
            return new ArrayList<Book>(); // возвращаем пустой список при ошибке
        }
    }
    ...
    public void setMarkList(List<WriteMark> markList) { this.markList = markList; }
}
